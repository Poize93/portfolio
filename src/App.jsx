...
function getDriveThumbnailUrl(previewUrl, size = "w400") {
  ...
  return m
    ? `https://drive.google.com/thumbnail?id=${m[1]}&sz=${size}`
    : previewUrl;
}
  ...
  return (
    ...
    >
      <div className="video-modal-box" onClick={(e) => e.stopPropagation()}>
        ...
        <div
          className={`video-modal-player ${isImage ? "video-modal-player--image" : ""}`}
        >
          {isImage ? (
            <img
              src={imageUrl}
...
function CarouselSection({
  ...
  console.log(duplicated?.length, "fsdfsdfdsfdfds", title);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const newScroll = Math.max(
        0,
        Math.min(el.scrollWidth - el.clientWidth, startScrollLeft + dx),
      );
        ...
        <div className="work-carousel-track work-carousel-track--auto">
          {isCard &&
            duplicated.map((item, i) => (
              ...
              >
                <div
                  className={`project-card-image ${item.video ? "project-card-image--video" : ""}`}
                >
                  {item.video ? (
                    <>
                      {failedThumbnails.has(`v-${item.id}-${i}`) ? (
                        ...
                        <img
                          src={
                            item.thumbnail || getDriveThumbnailUrl(item.video)
                          }
                          alt=""
                          loading="lazy"
                          className="project-card-thumbnail"
                          onError={() =>
                            markThumbnailFailed(`v-${item.id}-${i}`)
                          }
                        />
                      )}
                      <span className="project-card-play-hint">
                    ...
                    <>
                      {failedThumbnails.has(`i-${item.id}-${i}`) ? (
                        ...
                        <img
                          ...
                          loading="lazy"
                          onError={() =>
                            markThumbnailFailed(`i-${item.id}-${i}`)
                          }
                        />
                      )}
                    </>
          ...
          {isSkill &&
            duplicated.map((item, i) => (
              <div key={`skill-${item.id}-${i}`} className="skill-tile">
                {item.name}
              </div>
            ))}
        </div>
      ...
      <main className="main">
        <section className="hero">
          ...
          <p className="hero-tagline">
            A Motion , Graphic & Ui Designer crafting purposeful visuals that
            move , connect and communicate.
          </p>
          <a
            href="#contact"
            className="hero-cta"
        ...
        <section className="section section--mint" id="about">
          ...
          <p className="section-text section-text--large">
            I work across motion graphics, UI design, and digital branding,
            using tools like After Effects, Illustrator, Premiere Pro,
            Photoshop, and Figma. I love blending creativity with
            strategy-because good design shouldn’t just look beautiful, it
            should communicate.
          </p>
          <p className="section-text">
            From social campaigns to UI experiences and brand visuals, I design
            with intention, clarity, and a touch of bold storytelling.{" "}
          </p>
          {/* <p className="section-text">
        ...
        <section className="section section--cream" id="contact">
          <h2 className="section-title section-title--large">Let's Create</h2>
          <p className="section-text">have a Project in mind?</p>{" "}
          <p className="section-text">
          ...
          <div className="contact-links">
            ...
            <a
              href="https://www.linkedin.com/in/shweta-dwivedi-6075b1224?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=ios_app"
              target="_blank"
